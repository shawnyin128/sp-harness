...
| **Supersession tracking** | When a feature replaces an existing one (`supersedes` field in features.json), brainstorming forces a Supersession Plan listing source files AND runtime artifacts (caches, indexes, model files, DB tables, config keys). writing-plans generates mandatory cleanup tasks BEFORE new implementation. sp-evaluator verifies cleanup. sp-feedback audits archived supersession records for stale artifact drift. Prevents the "new code reads old knowledge base" bug class. |
