#!/usr/bin/env python3
"""Backfill display_name on existing .claude/todos.json entries.

Usage:
  backfill_display_names.py [path-to-todos.json]

Defaults to .claude/todos.json in the current working directory.
...
DEFAULT_PATH = Path(".claude/todos.json")
...
def main(argv: list[str]) -> int:
    ...
    for entry in entries:
        ...
        if not derived or not derived.strip():
            print(
                f"error: cannot derive display_name for todo "
