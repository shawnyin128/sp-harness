...
## Divergence Handling
[For each divergence point from the spec analysis, the specific fallback
logic that must be implemented — detection, recovery, safe stop]
```
...
## Feature: [feature-id]
## Generated by: Planner (iteration 1)

## Functional Criteria
- [ ] [Specific testable behavior from feature steps]
...
## Divergence Criteria
- [ ] [Fallback logic implemented for each identified risk]
- [ ] [Detection mechanism works as designed]

## Acceptance Threshold
All functional criteria must pass. Quality and divergence criteria:
at least N/M must pass (Planner specifies threshold).
```
...
### Functional Criteria
- [x] Behavior A — verified via test X
- [ ] Behavior B — FAIL: [specific reason]

### Quality Criteria
- [x] Code quality — clean, follows conventions
- [ ] Test coverage — FAIL: [missing coverage area]

### Divergence Criteria
- [x] Fallback for API timeout — implemented correctly
- [ ] LLM output validation — FAIL: [no schema check]

## Iteration Items (ITERATE only)

### Item 1
- **Location:** Task 2, file src/api/handler.py
- **Problem:** LLM response parsed without schema validation
- **Suggestion:** Add JSON schema check before processing
- **Priority:** must-fix

### Item 2
- **Location:** Task 1, file tests/test_chat.py
- **Problem:** No test for empty response edge case
- **Suggestion:** Add test that verifies fallback behavior
- **Priority:** should-fix

## Criteria Adjustments
[If the Evaluator modified eval-criteria.md, explain what changed and why.
Example: "Added criteria for error message clarity — original criteria
focused on functionality but user-facing errors need quality standards too."]

## Convergence Assessment
[Evaluator tracks whether issues are decreasing across iterations.
If iteration > 1: compare current items with previous iteration's items.
Report: "Converging" (fewer/same issues) or "Diverging" (new issues appearing)]
```
